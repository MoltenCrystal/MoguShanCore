    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Creature* target = GetHitCreature())
            if (target && !target->HasAura(SPELL_FLAMES))
            {
                ...
                target->CastSpell(target, SPELL_FLAMES);
...
{
    PrepareSpellScript(spell_q31450);
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_q31450::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};

// 122633 - Ghostly Skeleton Key
class spell_q32683_q32726_ghostly_skeleton_key : public SpellScript
{
    PrepareSpellScript(spell_q32683_q32726_ghostly_skeleton_key);

    void HandleDummyEffect()
    {
        if (Player* caster = GetCaster()->ToPlayer())
            if (caster->GetQuestStatus(32683) == QUEST_STATUS_INCOMPLETE || caster->GetQuestStatus(32726) == QUEST_STATUS_INCOMPLETE)
                caster->KilledMonsterCredit(70512);
    }

    void Register() override
    {
        OnCast += SpellCastFn(spell_q32683_q32726_ghostly_skeleton_key::HandleDummyEffect);
    }
};
...
enum CookingForKunzenBonfire
{
    SPELL_CREATE_STEW = 113950,
    QUEST_THE_MILE_HIGH_GRUB = 30331
};

class spell_cooking_for_kunzen_bonfire : public AuraScript
{
    PrepareAuraScript(spell_cooking_for_kunzen_bonfire);
    ...
    void HandleEffectPeriodic(AuraEffect const* aurEff)
    {
        ...
        if (!caster)
            return;
        if (caster->ToPlayer()->GetQuestStatus(QUEST_THE_MILE_HIGH_GRUB) != QUEST_STATUS_INCOMPLETE)
        {
            caster->CastStop();
            return;
        }
        if (aurEff->GetTickNumber() % 3)
            caster->CastSpell(caster, SPELL_CREATE_STEW);
    }

    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(spell_cooking_for_kunzen_bonfire::HandleEffectPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
    }
};
